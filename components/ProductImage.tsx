...
export function ProductImage({
  ...
  const isUploadedImage = currentImage?.startsWith("data:image/") || currentImage?.startsWith("http") || currentImage?.startsWith("/");
  const pattern =
    imageIndex % 3 === 1
    ...
    <div className={cn("relative overflow-hidden bg-[#eee6d8]", className)}>
      ...
      {isUploadedImage ? (
        <motion.img
          src={currentImage}
          alt={product.name}
          animate={floating ? { y: [0, -10, 0], scale: [1, 1.015, 1] } : undefined}
